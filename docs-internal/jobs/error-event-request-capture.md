...
```json
{
  ...
  "scope": [
    "gateway/migrations/0013_error_events_request_payload.sql",
  ...
  "acceptance": [
    ...
    "When on, the request body is buffered before handler dispatch, truncated to a configurable max (default 4 KiB) with a truncation marker, stored only on 4xx/5xx, and NULL on 2xx.",
...
1. Migration `0013_error_events_request_payload.sql` (nullable BYTEA, idempotent) → verify boot is a no-op on re-run.
