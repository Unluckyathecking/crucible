...
**Action:** Always attempt a pure `SELECT` first for lookups that are expected to exist 99% of the time, falling back to the `INSERT ... ON CONFLICT` only when the row is missing.
