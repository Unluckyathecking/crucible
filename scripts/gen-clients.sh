...
def py_method(op):
    ...
    if rtype == "Dict[str, Any]":
        # A JSON "null" body must decode to an empty dict, not None — mirrors
        # the Go generator's identical null-body regression-tested guarantee.
        ...
        lines.append("        if raw is None:")
        lines.append(f"            return cast({rtype}, {{}})")
        lines.append("        out = json.loads(raw)")
