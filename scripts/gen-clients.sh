...
# Connect-phase timeout regression test: urllib wraps this as
# URLError(reason=TimeoutError(...)), a different path than the bare
# TimeoutError raised for read-phase timeouts (see py_client_request_helper) —
# both must map to a retryable ApiError. Mocked rather than triggered over a
# real socket: a genuine connect-timeout is environment-dependent (routing,
# firewalls) and not reliably reproducible in CI.
connect_timeout_test_py = ""
if first_typed_op:
    call_connect_timeout = f"c.{first_typed_op.op_id}(" + ", ".join(py_test_call_args(first_typed_op)) + ")"
    connect_timeout_test_py = "\n\n\n" + "\n".join([
        f"def test_{first_typed_op.op_id}_wrapped_connect_timeout_is_retryable():",
...
# Error-body read-timeout regression test: a stalled non-2xx body read raises
# TimeoutError from inside the HTTPError branch of _request, a third distinct
# timeout path (alongside the bare read-phase and wrapped connect-phase
# cases above) that must also map to a retryable ApiError rather than leak.
error_body_timeout_test_py = ""
if first_typed_op:
    call_error_body_timeout = f"c.{first_typed_op.op_id}(" + ", ".join(py_test_call_args(first_typed_op)) + ")"
    error_body_timeout_test_py = "\n\n\n" + "\n".join([
        f"def test_{first_typed_op.op_id}_error_body_read_timeout_is_retryable():",
        "    class _StallingBody:",
