...
func TestReserve_BelowCapAdmits(t *testing.T) {
	...
	tr := New(rdb)
	for i := 0; i < 5; i++ {
		ok, _, _, _, err := tr.Reserve(context.Background(), cust, 10)
		if err != nil || !ok {
...
func TestReserve_OverCapRejects(t *testing.T) {
	...
	const cap = 3
	for i := 0; i < cap; i++ {
		ok, _, _, _, _ := tr.Reserve(context.Background(), cust, cap)
		if !ok {
...
func TestReserve_CapZeroMeansUnlimited(t *testing.T) {
	...
	tr := New(rdb)
	for i := 0; i < 50; i++ {
		ok, _, _, _, err := tr.Reserve(context.Background(), cust, 0)
		if err != nil || !ok {
...
func TestReserve_PerCustomerIsolation(t *testing.T) {
	...
	const cap = 3
	for i := 0; i < cap; i++ {
		ok, _, _, _, _ := tr.Reserve(context.Background(), a, cap)
		if !ok {
			t.Fatalf("a call %d should admit", i)
		}
	}
	// Customer A is exhausted; customer B should be untouched.
	ok, _, _, _, _ := tr.Reserve(context.Background(), b, cap)
	if !ok {
	...
	for ci, cust := range custs {
		for j := 0; j < callsPerCust; j++ {
			...
			go func(ci int, cust uuid.UUID) {
				defer wg.Done()
				ok, _, _, _, _ := tr.Reserve(context.Background(), cust, capPerCust)
				if ok {
...
func TestReserveThenRefund_NetsToZero(t *testing.T) {
	...
	tr := New(rdb)
	for i := 0; i < 20; i++ {
		ok, _, _, _, _ := tr.Reserve(context.Background(), cust, 100)
		if !ok {
			t.Fatalf("admit %d", i)
		}
		if err := tr.RefundAt(context.Background(), monthKey(cust, time.Now().UTC())); err != nil {
...
func TestReserve_UsesUTCKey(t *testing.T) {
	...
	tr := New(rdb)
	ok, returnedKey, _, _, err := tr.Reserve(context.Background(), cust, 5)
	if err != nil || !ok {
...
func TestReserve_CurrentValueAccurate(t *testing.T) {
	...
	const cap = 5
	for i := 0; i < cap; i++ {
		ok, _, current, _, err := tr.Reserve(context.Background(), cust, cap)
		if err != nil || !ok {
			t.Fatalf("call %d: ok=%v err=%v", i, ok, err)
		}
