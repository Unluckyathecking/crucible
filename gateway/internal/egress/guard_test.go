...
func TestBlockPrivateAddr(t *testing.T) {
	tests := []struct {
		...
		{"0.0.0.0/8 'this' network, not just 0.0.0.0 itself", "0.1.2.3:443", true},
		{"100.64.0.0/10 shared address space (CGNAT) low bound", "100.64.0.0:443", true},
		{"100.64.0.0/10 shared address space (CGNAT) high bound", "100.127.255.255:443", true},
		{"100.63.x is NOT in the CGNAT range", "100.63.255.255:443", false},
		{"100.128.x is NOT in the CGNAT range", "100.128.0.0:443", false},
		...
		{"IPv4-mapped IPv6 'this' network", "[::ffff:0.1.2.3]:443", true},
		{"IPv4-mapped IPv6 CGNAT", "[::ffff:100.64.1.1]:443", true},
